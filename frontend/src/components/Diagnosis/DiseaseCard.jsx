...
import { ChevronDown, ChevronUp, CheckCircle, X, Info, TrendingUp, Target, Clock, Brain } from 'lucide-react';

// Full screen modal for confidence breakdown
const ConfidenceBreakdownModal = ({ disease, rank, onClose }) => {
    const { translate } = useLanguage();
    if (!disease) return null;

    const colorMap = {
        1: { gradient: 'from-blue-600 to-blue-800', accent: '#2563eb', light: '#dbeafe', text: '#1e3a8a' },
        2: { gradient: 'from-emerald-600 to-teal-700', accent: '#10b981', light: '#d1fae5', text: '#065f46' },
        3: { gradient: 'from-amber-500 to-orange-600', accent: '#f59e0b', light: '#fef3c7', text: '#78350f' },
    };
    const c = colorMap[rank] || colorMap[1];

    // How we calculate each factor - explained clearly
    const factors = [
        {
            ...
            weight: '40%',
            description: 'Your reported symptoms matched the known symptom profile of this condition. Higher overlap = higher score.',
            barColor: c.accent,
        },
        {
            ...
            weight: '25%',
            description: 'The severity level you described aligns with how this condition typically presents.',
            barColor: c.accent,
        },
        {
            ...
            weight: '20%',
            description: 'The duration of your symptoms correlates with the typical progression timeline of this disease.',
            barColor: c.accent,
        },
        {
            ...
            weight: '15%',
            description: 'AI analyzed medical patterns from your full symptom description using Google Gemini AI.',
            barColor: c.accent,
        },
    ];
            ...
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                ...
                <div className={`bg-gradient-to-r ${c.gradient} p-7 rounded-t-3xl`}>
                    <div className="flex items-start justify-between">
                        ...
                        <button onClick={onClose} className="p-2 bg-white/20 hover:bg-white/30 rounded-xl transition-colors ml-4 flex-shrink-0">
                ...
                <div className="p-7 space-y-6">
                    ...
                    <div className="rounded-2xl p-4 border-2" style={{ backgroundColor: c.light, borderColor: c.accent + '40' }}>
                        <p className="text-sm font-bold" style={{ color: c.text }}>
                            📊 {translate('howWeCalculated') || 'How we calculated this score'}
                        </p>
                        <p className="text-xs mt-1" style={{ color: c.text + 'cc' }}>
                            Our AI analyzed 4 key factors with different weights to arrive at the final {disease.confidence}% confidence score.
                        </p>
                    </div>
                            ...
                            <div key={i} className="bg-slate-50 rounded-2xl p-5 border border-slate-100">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center justify-between mb-0.5">
                                            ...
                                            <div className="flex items-center gap-2">
                                                <span className="text-xs text-slate-400 font-bold">Weight: {factor.weight}</span>
                                                <span className="font-black text-base" style={{ color: factor.barColor }}>{factor.score}%</span>
                                            </div>
                                        </div>
                                ...
                                <div className="w-full h-2.5 bg-slate-200 rounded-full overflow-hidden">
                                    <div className="h-full rounded-full transition-all duration-700"
                                        style={{ width: `${factor.score}%`, backgroundColor: factor.barColor }} />
                                </div>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="rounded-2xl p-5 border-2" style={{ backgroundColor: c.light, borderColor: c.accent }}>
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="font-black text-slate-800">Weighted Final Score</p>
                                <p className="text-xs text-slate-500 mt-0.5">40% × {factors[0].score} + 25% × {factors[1].score} + 20% × {factors[2].score} + 15% × {factors[3].score}</p>
                            </div>
                            <span className="text-3xl font-black" style={{ color: c.accent }}>{disease.confidence}%</span>
                        </div>
                    </div>

                    <button onClick={onClose} className="w-full py-4 font-black text-white rounded-2xl transition-all active:scale-95"
...
const DiseaseCard = ({ disease, rank, isPrimary }) => {
    ...
    const reasons = Array.isArray(disease.reasoning) ? disease.reasoning : disease.reasoning ? [disease.reasoning] : disease.reason ? [disease.reason] : [];
            ...
            <div className={`bg-white rounded-3xl border-2 ${cs.border} shadow-sm hover:shadow-lg transition-all ${rank === 1 ? 'ring-2 ' + cs.ring : ''} overflow-hidden`}>
                ...
                <div className={`bg-gradient-to-r ${cs.bg} p-5`}>
                    <div className="flex items-center justify-between gap-4">
                        <div className="flex items-center gap-4 flex-1 min-w-0">
                            ...
                            <div className="flex-1 min-w-0">
                                {rank === 1 && <div className="inline-flex items-center gap-1 px-3 py-1 bg-blue-600 text-white rounded-full text-[10px] font-black mb-2">{translate('mostLikely')}</div>}
                                <h3 className="text-lg font-black text-[#0f172a] leading-tight">{disease.name}</h3>
                            </div>
                        </div>
                        {/* Clickable confidence score button */}
                        <button onClick={() => setShowBreakdown(true)}
                            className={`flex-shrink-0 ${cs.confBg} text-white rounded-2xl px-5 py-3 flex flex-col items-center hover:opacity-90 active:scale-95 transition-all shadow-md cursor-pointer`}
                            title="Click to see how this score was calculated">
                            <span className="text-2xl font-black">{disease.confidence}%</span>
                            <span className="text-[10px] font-bold opacity-80">Click for details</span>
                        </button>
                    </div>
                </div>
                ...
                <div className="px-5 pb-5">
                    <button onClick={() => setExpanded(!expanded)}
                        className="w-full flex items-center justify-between py-3 px-4 mt-4 bg-slate-50 hover:bg-slate-100 rounded-2xl transition-colors text-left border border-slate-100">
                        <div className="flex items-center gap-3">
                            <div className="w-7 h-7 bg-blue-100 rounded-lg flex items-center justify-center">
                                <Info className="w-4 h-4 text-blue-600" />
                            </div>
                            <span className="font-bold text-slate-700 text-sm">{expanded ? translate('hide') : translate('show')} {translate('clinicalReasoning')}</span>
                        </div>
