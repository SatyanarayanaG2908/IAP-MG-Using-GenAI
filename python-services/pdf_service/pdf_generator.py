...
# ── Font Registration (Cross-Platform Fix) ─────────────────────────────────────
def get_font_directories():
    """Returns font directories based on OS - Windows, Linux, Mac support"""
    font_dirs = []
    ...
    if sys.platform == 'win32':
        ...
        # Linux / Ubuntu / Deployment servers
        font_dirs.extend([
        ...
        for font_dir in font_dirs:
            ...
            if os.path.exists(path):
                try:
                    ...
                    print(f"✅ Registered font: {name} from {font_dir}")
                    count += 1
                    registered = True
                    break
                    ...
                    print(f"⚠️ {name}: {e}")
        if not registered:
            print(f"⚠️ Font not found on this system: {file} (will use Helvetica fallback)")
...
def get_font_for_language(language: str) -> str:
    ...
    try:
        ...
        print(f"⚠️ Font '{font_name}' not available, falling back to Helvetica")
        return "Helvetica"
...
def generate_confidence_charts(diseases: list) -> bytes:
    ...
    wedges, texts, autotexts = ax1.pie(
        confidences, labels=None, colors=chart_colors, autopct='%1.0f%%',
    ...
    bars = ax2.barh(range(len(diseases)), confidences, color=chart_colors,
...
def generate_pdf_report(user_data: dict, symptoms: str, analysis_result: dict, language: str) -> str:
    ...
    primary_conf = next((d.get('confidence',0) for d in diseases if d.get('name') == final_disease),
                        diseases[0].get('confidence',0) if diseases else 0)
    primary_data = [[
    ...
    # SECTION 6 — ANALYSIS RESULTS TABLE
    ...
    if medicines:
        ...
        if med_rows:
            mt = Table(med_header + med_rows,
                       colWidths=[1.9*inch, 0.85*inch, 1.15*inch, 0.95*inch, 2.15*inch],
                       repeatRows=1)
    ...
    if recovery and isinstance(recovery, dict):
        ...
        if recovery.get('duration'):
            rec_rows.append([Paragraph("⏱ <b>Expected Duration</b>",    small_style), Paragraph(str(recovery['duration']),   normal_style)])
        if recovery.get('confidence'):
            rec_rows.append([Paragraph("📊 <b>Recovery Confidence</b>",  small_style), Paragraph(str(recovery['confidence']), normal_style)])
        if recovery.get('timeline'):
            rec_rows.append([Paragraph("📋 <b>Timeline Notes</b>",       small_style), Paragraph(str(recovery['timeline']),   normal_style)])
